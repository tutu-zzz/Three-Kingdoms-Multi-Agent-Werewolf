...
def majority_vote_cn(votes: Dict[str, str]) -> tuple[str, str]:
    """中文版多数投票统计"""
    '''
    --votes 是一个字典，字典的键和值都是字符串类型。键表示投票人的名字，值表示他们投的选项
    --函数的返回类型是一个元组，包含两个字符串。第一个字符串是获得最多投票的选项，第二个字符串是该选项的投票数
...
def check_winning_cn(alive_players: List[AgentBase], roles: Dict[str, str]) -> Optional[str]: # Optional[X]表示一个值可以是X类型，或者是None
    """检查中文版游戏胜利条件"""
...
class GameModerator(AgentBase):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.name = "游戏主持人"
        self.game_log: List[str] = [] # 准备一个空的游戏日志列表
    ...
    async def vote_result_announcement(self, vote_out: str, vote_count: int) -> Msg:
        """投票结果公告"""
        content = f"投票结果：{vote_out}以{vote_count}票被淘汰出局。"
        return await self.announce(content)
